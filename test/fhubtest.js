...
//                         return instance.getProjectInfo.call(projectAddress, {from: accounts[0]}).then(PArray=>{
//                             amount_Raised = PArray[3];
//                             amount_Raised=Number(amount_Raised);
//                             assert.isAtLeast(acc1_initial_balance-acc1_current_balance,amount,"test 1: Amount wasn't correctly taken from the sender");
//                             console.log("Raised amount"+amount_Raised);

//                          //   assert.isAtLeast(amount_Raised,(initial_amount+amount),"test 1: Funded amount has not been traffered correctly");
...
contract('FundingHub', accounts => {
  ...
  it("Should refund all the received amounts to their repsective donors", function() {
    ...
    return FundingHub.deployed().then(instance=>{
        instance.CreateP("testP", accounts[0], 2000000000000000000, 600000000000000, {from: accounts[0], gas: 3000000}).then(tx=>{

            instance.latestProject().then(projectAddress=>{
                instance.getProjectInfo.call(projectAddress, {from: accounts[0]}).then(Initial_PArray=>{
                    //console.log(Initial_PArray);
              ...
              // .then(account1=>{
                 ...
                 //   console.log("balance"+projectAddress.balance);
                    instance.getProjectInfo.call(projectAddress, {from: accounts[0]}).then(PArray=>{
                       // console.log(PArray);
                            ...
                            assert.equal(amount_Raised,(initial_amount+amount),"test 1: Funded amount has not been traffered correctly");

                            initial_amount = amount_Raised;
                            acc1_initial_balance =  acc1_current_balance;
                           ...
                           // console.log(ref);
                            ...
                            amount_Raised = PArray[3];
                            assert.equal(amount_Raised,initial_amount-amount,"test 1: The Donor has not been refunded as it should");
                            assert.isAbove( acc1_current_balance,acc1_initial_balance, " test 1: refund has not been done correctly");
                         //   console.log("acc1_current_balance"+acc1_current_balance);
                           // console.log("acc1_initial_balance"+acc1_initial_balance);
